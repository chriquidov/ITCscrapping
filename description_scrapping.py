...
def transform(soup, joblist):
    ...
    divs2=soup.find_all('div' , class_='mosaic mosaic-provider-jobcards')
    for item in divs2:
        link_to_description = get_link_to_full_description(item)
        ...
        job = {
            ...
            'job_type': job_type,
            'summary': summary,
            'detailed_description': link_to_description,
        }
        joblist.append(job)
    return
...
def get_link_to_full_description(item):
    """Param: item in page's soup
        returns the job type of the specific job offer if exists"""
    try:
        return item.find('href').text.strip()
    except:
        return 'Nan'
